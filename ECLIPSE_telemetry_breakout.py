...
Date:   2024-01-02, Created
        ...
        2026-06-24, Sped up HRT sync-word scan and instrument-ID dispatch;
                    fixed apid NameError-masking bug in breakout_suvm_packet 
Version: 1.1
...
def breakout_suvm_packet(sin: list[bytearray]) -> list[suvm_packet()]:
    ...
    return out
#
_SYNC_BYTES = b'\x1d\xfc\xcf\x1a'   # little-endian for 0x1acffc1d

# inst_id byte patterns (raw bytes(hdr[4:8])), avoiding hex-string conversion
_TIP1_M0  = b'\x01\x00\x75\x00'
_TIP1_AUX = b'\x01\x00\x05\x00'
_TIP1_M1  = b'\x01\x00\xba\x00'
_MIP3_M0  = b'\x03\x00\x75\x00'
_MIP3_AUX = b'\x03\x00\x05\x00'
_MIP3_M1  = b'\x03\x00\xba\x00'
_TIP5_M0  = b'\x05\x00\x75\x00'
_TIP5_AUX = b'\x05\x00\x05\x00'
_TIP5_M1  = b'\x05\x00\xba\x00'
_MIP7_M0  = b'\x07\x00\x75\x00'
_MIP7_AUX = b'\x07\x00\x05\x00'
_MIP7_M1  = b'\x07\x00\xba\x00'
_ANALOG1  = b'\x09\x00\x40\x00'
_ANALOG2  = b'\x0a\x00\x40\x00'
...
def breakout_hrt_packet(fd: list[bytes], tc: list[float], te: list[float]) -> list[eclipse_packet()]:
    ...
    i = 0
    while i <= n:
        # Jump straight to the next sync word at the C level instead of
        # stepping through every non-matching byte in Python.
        i = fd.find(_SYNC_BYTES, i)
        if i == -1 or i + 12 > n:
            break
        hdr = fd[i:i+12]
        inst_id = hdr[4:8]
        if inst_id[0:2] == b'\x09\x00' and hdr[6:8] != b'\x40\x00':
            fault_bytes = hdr[6:8].hex()
            print(f'Faulty instrument HRT Packet ID: {fault_bytes}')
        #
        pkt_length  = hdr[6] | (hdr[7] << 8)
        time_tag_moe = int.from_bytes(hdr[8:12], 'little')  # Integer GPS time tag in HRT Frame, currently unused
        tidx = i // 1270
        ...
        try:
            if inst_id == _TIP1_M0 and data_packet[105:109].decode('utf-8') == 'AF09':
        ...
        if inst_id == _TIP1_AUX:   # XIP 1 Aux (5 bytes)
            out.tip1_aux.append([time_tag, data_packet])
        if inst_id == _TIP1_M1:    # XIP 1 M1  (186 bytes)
            out.tip1_m1.append([tc[tidx], te[tidx], time_tag, data_packet])
        if inst_id[0:2] == b'\x02\x00':   # SUVM 2
            out.suvm2.append([tc[tidx], te[tidx], time_tag, data_packet])
        try:
            if inst_id == _MIP3_M0 and data_packet[105:109].decode('utf-8') == 'BF02':
        ...
        if inst_id == _MIP3_AUX:   # XIP 3 Aux (5 bytes)
            out.mip3_aux.append([time_tag, data_packet])
        if inst_id == _MIP3_M1:    # XIP 3 M1  (186 bytes)
            out.mip3_m1.append([tc[tidx], te[tidx], time_tag, data_packet])
        if inst_id[0:2] == b'\x04\x00':   # SUVM 4
            out.suvm4.append([tc[tidx], te[tidx], time_tag, data_packet])
        try:
            if inst_id == _TIP5_M0 and data_packet[105:109].decode('utf-8') == 'AF08':
        ...
        if inst_id == _TIP5_AUX:   # XIP 5 Aux (5 bytes)
            out.tip5_aux.append([time_tag, data_packet])
        if inst_id == _TIP5_M1:    # XIP 5 M1  (186 bytes)
            out.tip5_m1.append([tc[tidx], te[tidx], time_tag, data_packet])
        if inst_id[0:2] == b'\x06\x00':   # SUVM 6
            out.suvm6.append([tc[tidx], te[tidx], time_tag, data_packet])
        try:
            if inst_id == _MIP7_M0 and data_packet[105:109].decode('utf-8') == 'BF03':
