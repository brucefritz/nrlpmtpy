...
File contents:
    refgeoid() - function
    Inputs: GEOCENTRIC latitude (NOT GEODETIC)
...
;
; PURPOSE:
; Determine the GEODETIC sub-tangent location.  This is located somewhat off
; the radius vector to the tangent point.  This allows the latitude, longitude
; and radius of the Earth to be calculated properly.
;
...
;				Research Laboratory, Code 7607.
    ...
    STPFUNC()
;    IDL Version 1.0: Scott Alan Budzien, NRL Code 7607,  09/07/1993
; PURPOSE:  function for determining where the surface normal
;           passes through a particular (r,z) location.
; --> A very ugly function for determining where the surface normal
; passes through a particular (r,z) location.
; 
; MODIFICATION HISTORY:
    Updated for Python: Bruce A. Fritz, NRL Code 7634, 2024-02-27

    TPHFUNC()
;	  IDL Version 1.0: Scott Alan Budzien, NRL Code 7607,  09/08/1993
; PURPOSE:  		function for determining where the line-of-sight
...
; MODIFICATION HISTORY:
    ...
    TPHFUNC2()
;	IDL Version 0.9: Scott Alan Budzien, NRL Code 7607,  09/08/1993 tphfunc
;	IDL Version 1.0: Scott Alan Budzien, NRL Code 7607,  01/01/2001
; PURPOSE: Function for determining where the line-of-sight
; 		is a specified geocentric distance above the surface.
...
; MODIFICATION HISTORY:
    ...
    regfal() (Regula-Falsi 1-D root finder)
        From: Version 1.0: Scott Alan Budzien, NRL Code 7607, 01/07/1997
; INPUT ARGUMENTS: Variable Name         Type                    Description
;    funcname        = string containing name of function
;    a_in, b_in      = bounds of region containing a root of "funcname"
;    tol (optional)  = convergence criterion in the value of x.  Default = 1e-5
;    nmax (optional) = maximum number of iterations allowed.     Default = 200.
...
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
    An example from ChatGPT for calculating SubTanPoint
...
def refgeoid(lat_geoc_deg):
    ...
    # Convert the geocentric latitude to the parameter angle describing the
    # geoid ellipse. The ellipsoid can be expressed in cylindrical coordinates.
...
def tanpoint(spv_gei, look_gei, zmin=None, spherical_earth=False):
    global tph_ae, tph_be, tph_b, tph_m, tph_zmin
    # Define the normal vector to the Satellite/LOS/Earth plane using 
    # using the cross product. Passes through Earth center
    nvx = look_gei[1,:] * spv_gei[2,:] - look_gei[2,:] * spv_gei[1,:]
    nvy = look_gei[2,:] * spv_gei[0,:] - look_gei[0,:] * spv_gei[2,:]
    nvz = look_gei[0,:] * spv_gei[1,:] - look_gei[1,:] * spv_gei[0,:]
    ...
    if len(sel) > 0:
        ...
        cosph[sel] = 1.0
    # 
    # The intersection of a plane passing through the origin with an ellipsoid
    # is an ellipse.  The major axis of the ellipse is still the Earth's
    # equatorial radius, but the minor axis is in-between the equatorial and
    # polar radii. The offset of the normal from the z-axis is the same as
    ...
    # Compute transformed coordinates for SC location and LOS direction
    px = spv_gei[0,:] * cosph * costh + spv_gei[1,:] * sinph * costh - spv_gei[2,:] * sinth
    py = -spv_gei[0,:] * sinph + spv_gei[1,:] * cosph
    pz = spv_gei[0,:] * cosph * sinth + spv_gei[1,:] * sinph * sinth + spv_gei[2,:] * costh
    # 
    tpunorm = np.linalg.norm(look_gei, axis=0)
    ux = (look_gei[0,:] * cosph * costh + look_gei[1,:] * sinph * costh - look_gei[2,:] * sinth) / tpunorm
    uy = (-look_gei[0,:] * sinph + look_gei[1,:] * cosph) / tpunorm
    uz = (look_gei[0,:] * cosph * sinth + look_gei[1,:] * sinph * sinth + look_gei[2,:] * costh) / tpunorm
    # 
    ...
    if len(sel) > 0:
        ...
        # We want to find theta where r - Re = zmin
            if ux[sel[i]] != 0.:
                ...
                # print(tanx)
                # print(px)
                yrange = [tph_b + tph_m * xrange[0], tph_b + tph_m * xrange[1]]
                # print(xrange, yrange)
            else:
            ...
            if ux[sel[i]] != 0.:
                thetap[i] = regfal('tphfunc', thmin, thmax, tol=1e-7)
                tanrp[i] = tph_b / (np.sin(thetap[i]) - tph_m * np.cos(thetap[i]))
            else:
                thetap[i] = regfal('tphfunc2', thmin, thmax, tol=1e-7)
                tanrp[i] = tph_b / np.cos(thetap[i])
    ...
    # 
    output = np.zeros((3,np.size(tx)))
    output[0,:] = tx
    output[1,:] = ty
    output[2,:] = tz
    # plt.plot(list(range(len(tz))), tz)
    return output

def subtanpt(invector,tol=None,quick=None):
    global stp_ae, stp_be, stp_tr, stp_tz
    ae = refgeoid(0.)
    be = refgeoid(90.)
    # 
    ...
    # 
    stp_tr = trad.copy()   # place entire array in common block
    stp_tz = invector[2,:]
    stp_ae = ae.copy()
    stp_be = be.copy()
    if quick is not None: # single iteration of Regula-Falsi
        ...
        for i in range(np.size(psi)):
            stp_tr = trad[i]
            stp_tz = invector[2,i]
            psi[i] = regfal('stpfunc',psi_lo[i],psi_hi[i])
...
def stpfunc(psi):
    c = np.cos(psi)
    s = np.sin(psi)
    return (stp_be**2)*c*s - stp_be*stp_tz*c + stp_ae*stp_tr*s - (stp_ae**2)*s*c

def tphfunc(theta):
    c = np.cos(theta)
    s = np.sin(theta)
    psi = np.arctan2(tph_be*s, tph_ae*c)
    re = np.sqrt(tph_be**2 + (tph_ae**2-tph_be**2)*np.sin(psi)**2)
    return tph_b - (re + tph_zmin) * (s - tph_m*c)
...
def tphfunc2(theta):
    ...
    psi = np.arctan2(tph_be*s, tph_ae*c)
    re = np.sqrt(tph_be**2 + (tph_ae**2 - tph_be**2)*np.sin(psi)**2)
    return tph_b - (re + tph_zmin)*c

def regfal(func_name, a_in, b_in, tol=None, nmax=None):
